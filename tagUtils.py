...
disCoeffs = np.array([[-0.08],[0.3],[0],[0]])
# disCoeffs = np.array([-0.08,0.3,0,0])
edges = np.array([[0, 1],
                              [1, 2],
                              [2, 3],
...
def get_pose_point_noroate(H,tag_p):
    """
    将空间坐标转换成相机坐标但是不旋转
...
def get_distance(H,tag_p,t):
    points = get_pose_point_noroate(H,tag_p)
