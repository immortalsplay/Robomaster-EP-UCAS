...
#对每张图片，识别出角点，记录世界物体坐标和图像坐标
    ...
    #cv2.waitKey(1000)
'''
传入全部图片各自角点的三维、二维坐标，相机标定。
每张图片都有本身的旋转和平移矩阵，可是相机内参和畸变系数只有一组。
mtx，相机内参；dist，畸变系数；revcs，旋转矩阵；tvecs，平移矩阵。
'''
ret, mtx, dist, rvecs, tvecs = cv2.calibrateCamera(objpoints, imgpoints, gray.shape[::-1],None,None)
img = cv2.imread('E:/image/Ex4/1.jpg')
#注意这里跟循环开头读取图片同样，若是图片太大要同比例缩放，否则后面优化相机内参确定是错的。
img = cv2.resize(img,None,fx=0.5, fy=0.5, interpolation = cv2.INTER_CUBIC)
h,w = img.shape[:2]
'''
优化相机内参（camera matrix），这一步可选。
参数1表示保留全部像素点，同时可能引入黑色像素，
设为0表示尽量裁剪不想要的像素，这是个scale，0-1均可以取。
'''
